/* Copyright (C) 2018 Mediakanzlei - All Rights Reserved
 * You may not use, distribute and modify this code unless
 * you purchased the MK-liquid_sidebar license.
 *
 * You should have received a copy of the MK-liquid_ads license with
 * this file. If not, please write to: info@mediakanzlei.ch
 * or visit: www.mediakanzlei.ch

 Contributors
 --------------
 This is the official list of the liquid sidebar project contributors.

 * Mediakanzlei
   - Marc Friedmann
   - Daniel Burger
*/


// pushed the content container of respective website to the left side
document.querySelector("body, nav, footer")
.setAttribute("style", "margin-left: 0; margin-right: 0; left: 0; width: 994px");
// end

// create an iframe
createIframe();
// end

// function to create the iframe for displaying ad.
function createIframe() {
  var ifrm = document.createElement("div");
  ifrm.innerHTML = "<iframe src=\"https://mkcdn.ams3.digitaloceanspaces.com/mediakanzlei_liquid_sidebar_test/mediakanzlei_liquid_sidebar001/001_liquid_sidebar_de.html\" id=\"iframeId\" style=\"position: fixed; top: 0; right: 0; width: calc(100vw - 1009px); height: 100vh; z-index: 9999999; border: none;\"></iframe>";
  document.querySelector("main").appendChild(ifrm);
  createButton();}
// end

// blick.ch specific code. The pages mediaquery creates odd behaviour on the nav bar, which causes the dropdown
// to shift to far to the right, when the ad is clicked away.
var css = '@media (min-width: 1300px) { .navigation-bar-level2 { margin-left: 0; margin-right: 0; left: 0; }}';
var style = document.createElement('style');
if (style.styleSheet) {
    style.styleSheet.cssText = css;
} else {
    style.appendChild(document.createTextNode(css));
}
document.getElementsByTagName('head')[0].appendChild(style);
// end

// creates the close button on the top right corner.
function createButton() {
  var closeButton = document.createElement("img");
  closeButton.setAttribute("id", "closebutton");
  closeButton.setAttribute("src", "data:image/png;base64,");
  closeButton.setAttribute("style", "position: fixed; top: 0; right: 0; width: 1vw; height: 1vw; margin-right: 0.5%; margin-top: 0.5%; z-index: 99999999; cursor: pointer");
  document.body.appendChild(closeButton);
// adds a click function to the close button, removes all elements from body.
  document.getElementById("closebutton").addEventListener("click", function() {
    var elem = document.querySelector('#iframeId');
    elem.parentNode.removeChild(elem);
// re-centers the website content container
    document.querySelector("body, nav, footer")
    .setAttribute("style", "margin: auto; width: 100vw");
// removes the button element from the DOM
    var btn = document.querySelector('#closebutton');
    btn.parentNode.removeChild(btn);
// removes the previously added CSS from the head
    var css = '@media (min-width: 1300px) { .navigation-bar-level2 { left: calc((100% - 1300px) / 2); margin-left: 6px; }}';
    var style = document.createElement('style');
    if (style.styleSheet) {
        style.styleSheet.cssText = css;
    } else {
        style.appendChild(document.createTextNode(css));
    }
    document.getElementsByTagName('head')[0].appendChild(style);
}
)};
